...
// Our variables
// ———————————————
define	tileFlag	$6CB4		// Used to indicate that there is a pending PPU macro for the map. Changed from $6C00 -> $6CB4 to fix a bug of Dungeon palettes being overwritten with $00
...
bank 7; org $E901	// Good Hijack point to load more data/code to SRAM
	jsr MoveCodeToSRAM

org $FF43	// Moved the code originally at $FFD6 to $FF43 for compatibility with MMC5 (by Bogaa)
SafeBlipUpdate:
	jmp $7F30

bank 1; org $A450
MoveCodeToSRAM:	// Copy code to SRAM $7F30
	ldy.b #$00	// Make sure Y is $00
LoopRomToSRAM:
	lda.w SafeBlipUpdateCopy,y
	sta.w $7F30,y
	iny
	cpy.b #$20
	bne LoopRomToSRAM

	jsr $8D00	// Hijack, Fix original routine to load SRAM
...
// Calculate high byte of destination pointer
	tya
	adc.b #$13	// Add $13
...
bank 7; org $FF79	// 0x1FF89, Free space for MMC5. This was the MMC1 bank swap.
DoWholeMapHijack:
